...
namespace Webflow.API.Controllers.Institutes
{
    public partial class InstitutesController : ControllerBase
    {
        ...
        [HttpDelete("{id}")]
        public async Task<ActionResult<BaseResponse<string>>> Delete(Guid? id, CancellationToken cancellationToken)
        {
            var result = await institutesService.Delete(id, cancellationToken);
