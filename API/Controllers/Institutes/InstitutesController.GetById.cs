...
namespace Webflow.API.Controllers.Institutes
{
    public partial class InstitutesController : ControllerBase
    {
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<BaseResponse<InstituteView>>> GetById(Guid? id, CancellationToken cancellationToken)
        {
            var result = await institutesService.GetById(id, cancellationToken);
